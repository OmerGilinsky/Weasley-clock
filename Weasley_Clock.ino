...
void update_display(int display, const char* image) {
  if (image == nullptr) {
    ...
    Serial.print("Trying to download ");
...
void play_sound(void* soundPtr) {
  const char* sound = (char*)soundPtr;
  ...
  if (httpPopResponseCode > 0) {
    ...
    if (String(status) == "ok") {
      ...
      if (String(eventType) == "move_clock_hand") {
        ...
        int screenNumber = doc["event"]["payload"]["screen"];
